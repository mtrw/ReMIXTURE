...
in_dm <- test_dm <- ReMIXTURE::ReMIXTURE_example_distance_matrix
test_dm_bad <- test_dm; test_dm_bad[1,1] <- 6
in_rt <- test_rt <- ReMIXTURE::ReMIXTURE_example_region_positions
r <- ReMIXTURE$new(test_dm,test_rt); r
r$distance_matrix[1:3,1:3]
...
#' Regionwise similarity analysis using a resampling/heirarchical clustering based method.
#'
#' @section Warning:
...
ReMIXTURE <- R6::R6Class(
  ...
  public = list(
    ...
    #' @param info_table A data.table describing the lat(y)/long(x)s of each region, with columns named "region", "x" and "y".
    ...
    initialize = function(distance_matrix,info_table=NULL){ #constructor, overrides self$new
      ...
      if( ncol(distance_matrix) != nrow(distance_matrix)){
        ...
        ce("\tDistance matrix is square ... PASS")
      }
      if( #lower triangular dm --- fill
        ...
        ce("\tFilling lower triangle of distance matrix ... NOT REQUIRED")
      }
      ...
      if( #upper triangular dm --- fill
        ...
        ce("\tFilling upper triangle of distance matrix ... NOT REQUIRED")
      }
